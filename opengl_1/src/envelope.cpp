...
void Envelope::computeNormals(){
    ...
    for (float t = path.getT0(); t < t1; t+=tDelta)
    {
        ...
        for (float a = tool->getA0(); a <= a1; a+=aDelta)
        {
            ...
            v1 = p1 + tool->getRadiusAt(a)*computeNormal(t, a, this);
            v2 = p2 + tool->getRadiusAt(a)*computeNormal(t, a+aDelta, this);
...
QVector3D Envelope::calcToolCenterAt(float t, float a)
{
    ...
    float a0 = tool->getA0();
    QVector3D center = getPathAt(t) + (a)*(toolMovement.getAxisDirectionAt(t));
...
QVector3D Envelope::getPathAt(float t){
    if (adjEnv != nullptr){
        ...
        return adjCenter + (rAdjEnv - tool->getRadiusAt(tool->getA0()))*computeNormal(t,adjEnv->tool->getA1(),adjEnv);
    } else {
        SimplePath path = toolMovement.getPath();
...
Vertex Envelope::calcEnvelopeAt(float t, float a)
{
    ...
    QVector3D normal = computeNormal(t, a, this);
    QVector3D posit = center + r*normal;

    return Vertex(posit, normal);
}

Vertex Envelope::calcEnvelopeAt(float t, float a, QVector3D adjCenter)
{
    ...
    float r = tool->getRadiusAt(a) + 0.0001; // + 0.0001 to avoid floating point weirdness
    QVector3D normal = computeNormal(t, a, adjEnv);
    QVector3D posit = center + r*normal;
...
QVector3D Envelope::computeNormal(float t, float a, Envelope* env)
{
    SimplePath path = env->toolMovement.getPath();
    QVector3D sa = env->toolMovement.getAxisDirectionAt(t);
    float a0 = env->tool->getA0();
    QVector3D st = path.getTangentAt(t).normalized() + (a-a0)*env->toolMovement.getAxisRateOfChange(t).normalized();
    //    qDebug() << "st: " << st;
    QVector3D sNorm = QVector3D::crossProduct(sa, st).normalized();

    // Calculate alpha, beta, gamma
    float alpha, beta, gamma;
    float ra = env->tool->getRadiusDerivativeWRTa(a);
    //    qDebug() << "ra: " << ra;
